...
import {
  deleteCarrierApplyDetails,
  getCarrierApplyDetails,
...
function ApplicationDetails() {
  ...
  const handleDelete = (id) => {
    deleteCarrierApplyDetails(id)
      .then((response) => {
        setCarrierApplyDetails(
          carrierApplyDetails.filter((val) => {
            return val._id != id;
          })
        );
      })
            ...
            <div className="table-contents | h-full overflow-y-auto border-[1px] border-black ">
              {carrierApplyDetails.map((value) => {
                ...
                var base64String = btoa(
                  new Uint8Array(resume.data.data).reduce(
                  ...
                  <table class="table-fixed border-fixed border border-slate-500 w-full">
                    <tbody>
                      <tr className="bg-[#ececec] text-[10px] md:text-[16px]">
                        ...
                        <td class="border border-slate-700 break-words text-center">
                          <a
                            className="text-[#0000ff] hover:underline"
                            download="resume"
                            href={`data:${resume.contentType};base64,${base64String}`}
                          >
                            {" "}
                            Download{" "}
                          </a>
                        ...
                        <td class="border border-slate-700 break-words text-center">
                          <button onClick={() => handleDelete(_id)}>
                            {" "}
                            <img className="w-[20px]" src={del} alt="" />{" "}
                          </button>
                        </td>
                      </tr>
                    </tbody>
